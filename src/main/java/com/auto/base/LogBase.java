...
public class LogBase {
	...
	public void writeLog(HashMap<String, String[]> result,String reportName){
		try {
			...
			String modelPath = getClass().getResource("/log/seleniumReportModel.html").getFile();
			//读取log模板
			File input = new File(modelPath);
			...
			//fileName = System.getProperty("user.dir")+"\\resources\\log\\"+fileName+"\\"+ip+"\\"+reportName+logName+".html";
			fileName = "D:/resources/log/"+fileName+"\\"+ip+"\\"+reportName+logName+".html";
	...
	public void setTitle(Document doc,String reportName){
		...
		Element h1 = doc.select("H3").first();
		h1.text(reportName);
	}
}